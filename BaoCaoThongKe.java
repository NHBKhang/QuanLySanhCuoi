...
import static com.mycompany.mavenproject1.SanhCuoi.getCacSanh;
import java.io.File;
...
class DoanhThu extends BaoCaoThongKe {
    ...
    public DoanhThu(String maSanh, String ngay, int giaTien) {
        this.maSanh = maSanh;
        this.ngay = ngay;
...
class SapXep extends BaoCaoThongKe {
    ...
    public SapXep(String tenSanh, String maSanh, int dem) {
...
public class BaoCaoThongKe {
    ...
    public void init() {
        ...
        try ( Scanner a = new Scanner(getFile())) {
            ...
            while (a.hasNext()) {
                ...
                DoanhThu h = new DoanhThu(maSanh, ngay, giaTien);
                ds.add(h);
                a.nextLine();
            }
        } catch (Exception ex) {
    ...
    public void timSapXep() {
        SanhCuoi sc = new SanhCuoi();
        DoanhThu dt = new DoanhThu();
        for (SanhCuoi s : getCacSanh()) {
            for (DoanhThu h : getDs()) {
                String catNgay[] = h.getNgay().split("/");
                    if (s.getMaSanh().equals(h.getMaSanh())) {
                        dem++;
                }
            }
            tenSanh=s.getTenSanh();
            maSanh=s.getMaSanh();
            SapXep p= new SapXep(tenSanh,maSanh,dem);
            sx.add(p);
        }
    }
